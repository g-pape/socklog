...
[tai64n](https://cr.yp.to/daemontools/tai64n.html) time-stamps prepended
to each message, use this `socklog-unix/log/run` script as example:
...
*socklog-ucspi-tcp* service on `10.0.0.16:10116` is (you need to have
...
A sample `socklog-notify/run` script that uses *wall* for notification
is:
